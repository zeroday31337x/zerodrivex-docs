...
export default async function DocPage({ params }: { params: { slug: string } }) {
  ...
  // Safe fallback values to prevent server errors
  const safeDoc = {
    ...doc,
  ...
  return (
    <div className="zdx-root">
      <style>{`
        ...
        .zdx-root::before { content:''; position:fixed; inset:0; background: repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(0,229,255,0.015) 2px, rgba(0,229,255,0.015) 4px); pointer-events:none; z-index:1; }
        .zdx-root::after { content:''; position:fixed; inset:0; background-image: linear-gradient(rgba(0,229,255,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(0,229,255,0.03) 1px, transparent 1px); background-size:40px 40px; pointer-events:none; z-index:0; }
        .wrapper { position:relative; z-index:2; max-width:860px; margin:0 auto; padding:80px 40px 120px; }
        ...
        h2 { font-family: monospace; font-size:22px; color:#fff; margin-bottom:24px; border-bottom:1px solid var(--border); padding-bottom:12px; }
      ...
      <div className="wrapper">
        ...
        <div className="cover">
          ...
          {safeDoc.summary && (
            <div className="cover-subtitle">{safeDoc.summary}</div>
          )}
        </div>
        ...
        {safeDoc.textContent && (
          <div className="abstract">
            {safeDoc.format === 'HTML' ? (
              <div dangerouslySetInnerHTML={{ __html: safeDoc.textContent }} />
            ) : (
